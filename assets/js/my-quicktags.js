...
QTags.addButton('clipMemo', 'クリップメモ', '<div class="clipMemo">', '</div>', '', 'クリップメモ', 1);
...
QTags.addButton('embedded_instagram', 'instagram', '<div class="instagram">', '</div>', '', 'instagram', 1);


// h2タイトルボタンの追加
QTags.addButton('h2_title', 'h2タイトル', function () {
    var title = prompt('見出しのタイトルを入力してください:');
    if (title) {
        QTags.insertContent('<h2>' + title + '</h2>');
    }
});

QTags.addButton('sc_Linkcard', 'リンクカード', function () {
    var url = prompt('URLを入力してください:');
    if (title) {
        QTags.insertContent('[sc_LinkcardEx url="' + url + '"]');
    }
});
QTags.addButton('sc_LinkcardEx', 'リンクカード（外部リンク）', function () {
    var url = prompt('URLを入力してください:');
    if (title) {
        QTags.insertContent('[sc_LinkcardEx url="' + url + '"]');
    }
