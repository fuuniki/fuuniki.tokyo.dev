...
jQuery('.js-yearHeading').click(function () {
  if (jQuery(this).next('.js-months').is(':visible')) { //既に開いている場所なら
    jQuery(this).next('.js-months').slideUp(300); //閉じる
    jQuery(this).children('.js-toggleIcon').removeClass('open'); //.acv_openを削除
  } else { //閉じている場所なら
    jQuery('.js-months').slideUp(); //全部閉じる
    jQuery('.js-monthlyArchive').find('.js-toggleIcon').removeClass('open'); //.acv_open全削除
    jQuery(this).next('.js-months').slideDown(300); //開く
    jQuery(this).children('.js-toggleIcon').addClass('open'); //.acv_open付加
  }
});
